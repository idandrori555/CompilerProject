...
globalThis.compile = function () {
  // Get the code from the editor
  const code = globalThis.getEditor().getValue();

  // Show compiling indicator
  const outputElement = document.getElementById("output");
  outputElement.innerHTML =
    '<div class="compiling-indicator"><div class="spinner"></div> Compiling...</div>';
...
globalThis.setupCompileButton = function () {
  // Create compile button
  const compileButton = document.createElement("button");
  compileButton.className = "compile-button";
  compileButton.innerHTML =
    '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg> Compile';
  compileButton.onclick = globalThis.compile;
